...
- `<tgbt-root>/oracles` を正本とし、中間ドキュメントのメンテナンスを避ける理由を記述している
...
- Codex CLI を用いて `tgbt` 自体を開発する際の `<tgbt-root>/oracles` の取り扱いを記述している
- `<tgbt-root>/oracles` と `tgbt` による開発対象の `<repo-root>/oracles` を混同しないための前提を記述している

# `oracles_docs_routing_policy.md`

- `<tgbt-root>/oracles/docs` 配下の `ROUTING.md` による目次・ルーティング規則を記述している
