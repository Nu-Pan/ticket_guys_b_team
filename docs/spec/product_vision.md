...
人間は `tgbt plan env` または `tgbt plan docs` で Plan を編集し、実行に進める意思決定は `tgbt run --plan-id ...` によって表す。MVP では独立した `approve` サブコマンドを持たず、「run を開始すること」が運用上の承認に相当する。
...
* worker 指示は `docs/spec/codex_worker_instructions.md` を正本とし、runtime では `.tgbt/instructions.md` を生成して参照する
* 要検討事項を増やさないため、worker 実行では skills と sub agent を使わない
