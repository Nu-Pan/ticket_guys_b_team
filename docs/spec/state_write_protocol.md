...
state-mutating command は、最初の publish を行う前に、少なくとも以下を済ませること。

* 変更対象の current state 読み取り
* 生成する candidate 一式の構築
* candidate 一式に対する validation
* repository lock が必要な command では、その lock 取得

これにより、「publish を始めてから validation failure で止まる」類の avoidable failure を減らす。
