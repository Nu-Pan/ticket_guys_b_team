...
* worker 実行では skills と sub agent を使用してはならない
...
`<repo-root>/.tgbt/instructions.md` は `docs/spec/codex_worker_instructions.md` を正本として runtime 生成されている前提で扱う。
live 実行時の wrapper は、process spawn の直前に `.tgbt/.codex/config.toml` と `.tgbt/instructions.md` を正本契約どおり再生成しなければならない。
既存 runtime file の内容をそのまま信頼して継承してはならない。
また、repository 直下 `.codex/` や `.tgbt/.codex/` 配下の他 file を worker runtime 入力として参照してはならない。
...
* 直前に再生成した `<repo-root>/.tgbt/.codex/config.toml` が存在し、profile `tgbt-worker` を定義していること
* 直前に再生成した profile `tgbt-worker` が `model_instructions_file = "<repo-root>/.tgbt/instructions.md"` を指していること
* 直前に再生成した `.tgbt/instructions.md` の内容契約が `docs/spec/codex_worker_instructions.md` と整合すること
