...
* live 記録の保存方法
* stub による replay 方法
* call purpose ごとの profile 選択
* session record の redaction 方針
* CLI / 状態遷移 / ファイル形式との接続点

本書は wrapper 境界の仕様に集中する。共通前提は `common_invariants.md`、運用 artifact は `operational_artifacts.md`、canonical markdown の commit 手順は `state_write_protocol.md` を参照する。
...
* live 実行では required profile set `tgbt-drafting` / `tgbt-worker` / `tgbt-review` のいずれかを call purpose に応じて明示指定しなければならない
* `tgbt` が Codex CLI を起動する実行では skills と sub agent を使用してはならない
* MVP では model / reasoning の正本を profile 側へ置き、wrapper request や環境変数 override で上書きしてはならない
...
* `codex_profile ∈ {tgbt-drafting, tgbt-worker, tgbt-review}`
...
### 7.3 `call_purpose` からの profile 解決

MVP では wrapper が `call_purpose` から codex profile を決定し、resolved runtime 値を内部で解決する。

canonical mapping は以下とする。

* `plan_drafting` → `tgbt-drafting`
* `ticket_planning` → `tgbt-drafting`
* `ticket_execution` → `tgbt-worker`
* `followup_planning` → `tgbt-review`

required profile set の各 profile は `.tgbt/.codex/config.toml` に明示的に記述されなければならない。
各 profile は少なくとも以下を持つ。
...
* wrapper request は `model` / `reasoning_effort` を受け取らないこと
* wrapper は `call_purpose` に対応する profile を 1 つに決定すること
* `model_reasoning_effort` は `minimal | low | medium | high | xhigh` のいずれかでなければならない
* resolved な `codex_profile` / `model` / `model_reasoning_effort` は strict replay identity の一部であり、session record に保存されること
* 環境変数による model / reasoning 上書きは行わないこと
...
* `<repo-root>/.tgbt/.codex/config.toml` が存在し、required profile set `tgbt-drafting` / `tgbt-worker` / `tgbt-review` を定義していること
* current call が使う profile が `model`、`model_reasoning_effort`、`approval_policy`、`sandbox_mode`、`model_instructions_file` を明示していること
* required profile set の各 profile が `model_instructions_file = "<repo-root>/.tgbt/instructions.md"` を指していること
* `.tgbt/instructions.md` の内容契約が `docs/spec/operational_artifacts.md` の Repo-Local Runtime File Contract と整合すること
...
* argv で `call_purpose` に対応する canonical profile を明示指定する
* runtime 指示は `model_instructions_file` 経由で `<repo-root>/.tgbt/instructions.md` を参照させる
* `--model` や per-call reasoning override により profile の resolved runtime を上書きしてはならない
