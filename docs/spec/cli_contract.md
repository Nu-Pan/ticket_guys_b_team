...
本書は CLI 利用者および CLI 実装者向けの契約文書である。共通前提は `common_invariants.md`、Plan / Ticket file format は `file_format.md`、運用 artifact は `operational_artifacts.md`、保存規則は `state_write_protocol.md`、Codex wrapper 契約は `codex_cli_wrapper.md` を参照する。
...
* `env` は repo-local Codex runtime を仕様適合状態へ整える独立サブコマンドとして公開する
* `run` 側は細分化しない
...
* 現在状態の正本、artifact 区分、採番の正本は `common_invariants.md` と `operational_artifacts.md` に従う
...
`repository root` の定義は `common_invariants.md` に従う。
...
state-mutating command の失敗後は `common_invariants.md` の restore 契約に従う。
...
`<repo-root>` 配下の Codex CLI の挙動に関わる一式を、`tgbt` の操作対象として仕様適合な状態へ one-shot で整える。
...
3. 現在状態の runtime 適合性と bootstrap diagnostics を観測する
4. 既に仕様適合なら補修は行わず、current invocation の env audit log を publish した後に成功終了する
5. 非適合なら自動修正対象の runtime file を deterministic に再生成する。`.tgbt/instructions.md` は `tgbt` 管理下の内容契約から生成し、追跡対象の単一 Markdown file をそのまま copy してはならない
6. 適合性を再検証する
7. `docs/spec/operational_artifacts.md` の `Env Audit Log File Format` に従って `.tgbt/logs/env-latest.jsonl` を publish する
8. runtime が仕様適合なら成功終了し、必要なら diagnostics を表示してよい
...
* 初回から仕様適合なら `Status: already_legal` を表示して成功終了する
* 補修後に適合化できた場合は `Status: legalized` を表示して成功終了する
...
指定した Plan を起点に、必要 Ticket の生成、Ticket 実行、実行結果 summary 反映、follow-up Ticket 生成を反復し、Plan を `settled` まで進めるか、途中失敗で停止する。
...
* `stub` のときは `common_invariants.md` と `codex_cli_wrapper.md` で定義する strict replay の前提が満たされていること
...
8. `ticket_execution` payload、実行結果 summary、実行 log、必要な session record を保存できた場合のみ、その Ticket を `done` にする
...
* 利用者は `common_invariants.md` の restore 契約に従って retry しなければならない
