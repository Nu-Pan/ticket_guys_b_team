...
* `plan` は親コマンドとし、PoC では `env` と `docs` のみを公開する
* `run` 側は細分化しない
...
`tgbt plan` は親コマンドであり、直接 state mutation を行わない。利用者は `env` または `docs` を明示しなければならない。
...
`<repo-root>` 配下の Codex CLI 設定と `AGENTS.md` を、tgbt の侵襲的アプローチに合わせて整合させる作業の Plan を新規作成する。
...
* `<repo-root>/.tgbt/plans/<plan_id>.md` を生成する
* 実行結果として少なくとも `Plan: <plan_id>`、`Plan revision: <plan_revision>`、`Updated: <absolute path>` を表示する
* 主要な session record 保存先を `Session record: <absolute path>` として表示する
...
* PoC では新規 Plan 作成専用とする
* `--plan-id`、`--codex-cli-mode`、追加の自然言語入力は受け付けない
