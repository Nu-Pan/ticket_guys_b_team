...
本書および `docs/spec/*.md` は、`ticket_guys_b_team` の正本仕様である。
既存実装、既存テスト、過去メモがこれらと矛盾する場合でも、仕様判断は `docs/spec/*.md` を優先する。
実装やテストが未追従である事実は、仕様変更の根拠ではなく、別途修正すべき差分として扱う。

現行仕様では、文書の正本配置を `doc/` ではなく `docs/` に統一する。
また、Codex CLI は対象 repository に侵襲的に合わせる方針を採り、user-global な `~/.codex` や repository 直下 `.codex/` には依存しない。
...
* `tgbt plan env` は repository lock を取得してから実行する
* `tgbt plan docs` は新規作成・既存更新の別を問わず repository lock を取得してから実行する
...
live 実行の Codex runtime は repository-local に固定する。
