...
* 同一 Plan に対して同時に複数の active run を持ってはならない
...
この遷移では、対象 Plan の現在 `plan_revision` に属する active Ticket 集合を破棄し、`plan_revision` を進めたうえで、以後の `run` で新しい Ticket 集合を作り直す。
...
同一 Plan に対する top-level `run` は 1 本だけ許可される。
