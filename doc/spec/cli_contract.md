...
Codex の live / stub 切り替えと業務レベル出力契約の詳細は `codex_cli_wrapper.md` を、front matter と state file の commit / recovery 規則は `state_write_protocol.md` を参照する。
...
* state-mutating command は新規 mutation を始める前に未完了 transaction の rollback-first recovery を完了しなければならない
...
* authoritative mutable state の commit は `state_write_protocol.md` に従う
* 複数ファイル mutation は transaction journal により論理的 atomicity を持たせる
...
lock を取得した state-mutating command は、新しい mutation を始める前に未完了 transaction を検出し、必要なら rollback-first recovery を実行しなければならない。
...
* 未完了 transaction が存在する場合は rollback-first recovery を完了できること
...
2. 未完了 transaction を検出し、必要なら rollback-first recovery を行う
3. `run_id` を採番する
4. Plan を `running` にする
5. Plan と active Ticket 群を見て、新規 Ticket が必要か判断する
6. 必要なら `ticket_planning` payload に従って新規 Ticket を 0 件以上作成する
7. active Ticket 集合の依存グラフを検証する
8. 依存解決済みの `todo` Ticket があれば、そのうち最小 `ticket_id` を選んで実行する
9. `ticket_execution` payload、実行結果サマリー、実行ログ、必要な session record を保存できた場合のみ、その Ticket を `done` にする
10. `done` Ticket に対して `followup_planning` を行う
11. 必要な follow-up Ticket を作成した後、元 Ticket を `settled` にする
12. 新規 Ticket 作成不要かつ active Ticket がすべて `settled` なら Plan を `settled` にして終了する
13. runnable な `todo` Ticket が 0 件で、かつ unsettled active Ticket が残る場合は run を失敗させる
14. `max_run_loop_count` または `max_new_ticket_count_per_run` を超える場合は run を強制停止する
15. repository lock を解放する
...
* source Ticket の settle と follow-up Ticket 作成は同一 transaction で commit しなければならない
...
* Ticket 実行が終了しても、`ticket_execution` payload、結果サマリー、実行ログ、必要な session record をすべて保存できるまでは、その Ticket を `done` にしてはならない
* Ticket 実行後に `done` 条件を満たす前で失敗した場合、その Ticket は `running` のまま残ってよい
* top-level `run` が途中失敗した場合、Plan は `running` のまま残してよい
* rollback-first recovery の対象は authoritative mutable state に限る
