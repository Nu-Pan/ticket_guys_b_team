...
本書でいう状態遷移は論理状態の定義であり、その可視化は `state_write_protocol.md` に従う transaction commit 単位で行う。

少なくとも以下を守らなければならない。

* `running -> draft` / `settled -> draft` の Plan 更新と active Ticket 退避・破棄は 1 transaction として可視化する
* Ticket の `done -> settled` と follow-up Ticket 作成は 1 transaction として可視化する
* transaction commit 前の candidate state を current state として解釈してはならない
...
この遷移では、repository lock を保持したうえで、対象 Plan の更新前 `plan_revision` に属する active Ticket 集合を破棄または退避し、`plan_revision` を進めたうえで、以後の `run` で新しい Ticket 集合を作り直す。これらは `state_write_protocol.md` に従う 1 transaction として commit しなければならない。
...
* source Ticket の settle と follow-up Ticket 作成は同一 transaction 内で commit されること
