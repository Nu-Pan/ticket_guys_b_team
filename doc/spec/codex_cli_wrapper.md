...
### 7.2 `model` / `reasoning_effort` の供給源
...
3 つの `call_purpose` について、Codex の最終メッセージは **単一の JSON object** でなければならない。
...
* `generated_artifacts` は repo-relative path を推奨する
* `followup_hints` は follow-up planning 用の補足情報であり、直接 Ticket を生成する権限は持たない

### 9.4 `followup_planning` payload
...
5. 最終メッセージ文字列を抽出する
6. business output を parse / validate する
7. redaction を適用する
8. canonical path に session record を保存する
9. redaction 済みの共通 result を返す
...
3 つの call purpose に対しては、`last_message_text` は redaction 後 `business_output` の canonical JSON serialization を保存することを推奨する。これにより parse 済み payload と raw text 表現がずれにくくなる。
