...
人間は `tgbt plan` で Plan を編集し、実行に進める意思決定は `tgbt run --plan-id ...` によって表す。MVP では独立した `approve` サブコマンドを持たず、「run を開始すること」が運用上の承認に相当する。
...
* **安全な自動化**: 未整理の要望から直接実装へ飛ばず、失敗時にも状態と記録を残し、repository 全体の同時 run を禁止すること
* **再現可能性**: stub によって過去の live 結果を再現でき、外部トークン消費や外部依存を避けられること
