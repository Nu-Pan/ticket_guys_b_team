"""Check <tgbt-root>/oracles ROUTING.md entries against the filesystem."""
...
def find_repo_root() -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "AGENTS.md").is_file() and (candidate / "oracles").is_dir():
            return candidate
    raise RuntimeError("Could not find <tgbt-root> containing AGENTS.md and oracles/")
...
def main() -> int:
    """`oracles/docs` 配下の routing entry と実体の差分を検査する。"""

    repo_root = find_repo_root()
    routing_root = repo_root / "oracles" / "docs"
    if not routing_root.is_dir():
        raise RuntimeError("Could not find <tgbt-root>/oracles/docs/")
    ...
    if not issues:
        print("OK: <tgbt-root>/oracles routing matches filesystem")
        return 0
