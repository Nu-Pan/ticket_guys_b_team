...
description: Maintain tgbt repo-local skills under `.agents/skills`. Use for creating, refactoring, validating, or refreshing skill metadata while aligning with tgbt oracles and upstream `$skill-creator`, without editing `CODEX_HOME`.
---
...
upstream の `$skill-creator` を必ず併用し、repo 固有ルールは `<tgbt-root>/AGENTS.md` と `<tgbt-root>/oracles`、Codex skill 一般の根拠は upstream skill-creator と OpenAI 公式 docs で確認する。

## Guardrails

- 新規・更新・メンテナンス対象は `<tgbt-root>/.agents/skills/<skill-name>` 配下に限定する。
...
- repo-local skill の原則は、`<tgbt-root>/oracles/docs/ROUTING.md`、`<tgbt-root>/oracles/docs/dev_rule/ROUTING.md`、`<tgbt-root>/oracles/docs/dev_rule/skills/ROUTING.md` を辿って、`<tgbt-root>/oracles/docs/dev_rule/skills/general_rule.md` と `<tgbt-root>/oracles/docs/dev_rule/skills/create_repo_local_skill.md` を参照する。
- path 表記、AI 管理ドキュメント、`<tgbt-root>/.venv`、依存導入、`<tgbt-root>/oracles` の扱いが論点になる場合だけ、対応する `<tgbt-root>/oracles/docs/dev_rule/*.md` を読む。
...
- install 先は常に `<tgbt-root>/.agents/skills` に固定する。
- upstream が既定で使う `$CODEX_HOME/skills` は使わない。
...
- `<tgbt-root>/oracles` の明示内容や `<tgbt-root>/AGENTS.md` と矛盾していないか。
...
- 更新した path、参照した主な `<tgbt-root>/oracles` / 公式 docs / upstream skill-creator、使った upstream script、validation 結果を報告する。編集しない review の場合は、更新なしと findings を明示する。
