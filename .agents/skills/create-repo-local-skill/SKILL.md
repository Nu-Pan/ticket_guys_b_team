...
- 新規・更新する skill は AI が読む作業指示であり、repo root の `<repo-root>/docs/` のような AI 管理の中間ドキュメントを作らない。
...
### 2. Check repo-local rules through ROUTING.md

- `AGENTS.md` を読んだうえで、`oracle/docs/ROUTING.md` から必要な階層の `ROUTING.md` を辿る。
- `oracle` 配下のファイル位置は各階層の `ROUTING.md` を正とし、古いパスや記憶で直接参照しない。
- repo-local skill の原則は `oracle/docs/dev_rule/ROUTING.md` で所在を確認してから `oracle/docs/dev_rule/codex_skill.md` を参照する。
- AI 管理ドキュメント禁止の前提は `oracle/docs/dev_rule/ROUTING.md` で所在を確認してから `oracle/docs/dev_rule/ai_docs.md` を参照する。
- `.venv` や依存導入が論点になる場合は `oracle/docs/dev_rule/ROUTING.md` で所在を確認してから `oracle/docs/dev_rule/environment.md` を参照する。
- `oracle` の扱い自体が論点になる場合は `oracle/docs/dev_rule/ROUTING.md` で所在を確認してから `oracle/docs/dev_rule/oracle.md` を参照する。
...
- 補足説明のためだけに skill 内へ `README.md`、`docs/`、`QUICK_REFERENCE.md` のような文書を追加しない。
...
3. `oracle/docs/ROUTING.md` から `oracle/docs/dev_rule/ROUTING.md` を辿り、必要な `oracle/docs/dev_rule/*` の制約を確認する
4. 必要なら `init_skill.py` を `--path .agents/skills` 付きで実行する
5. `SKILL.md` を repo-local の原則と、参照した `oracle` の明示内容に合わせて仕上げる
...
- 新規作成か既存更新かを明示する。
- 作成または更新した path と使った upstream script を最後に報告する。
