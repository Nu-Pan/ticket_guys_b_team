...
description: tgbt リポジトリ内の事柄について、実装・テスト・oracle を根拠に調査し、現状挙動や制約を説明するためのスキル。Use when Codex needs to answer repo-specific questions, trace behavior through code, tests, and oracle, compare conflicting evidence, summarize the current implementation, or perform scoped investigation without making product-level design decisions.
---
...
- 開発ルールや作業手順を知りたいときは、`AGENTS.md` と `oracle/dev_rule/*.md` を優先する。
- tgbt の仕様断片を知りたいときは、`oracle/ROUTING.md` と各階層の `ROUTING.md` から関連する `oracle/tgbt_spec/*.md` を辿る。
- `README.md` と `memo/**` は読まない。
- AI 管理ドキュメントや調査メモを作りたくなる作業では `oracle/dev_rule/ai_docs.md` を読み、永続化しない方針を守る。
...
3. 調査の起点として、以下のような代表ファイルから読む。
   ...
   - init 系: `src/cmd/init/service.py`
   - Plan 系: `src/cmd/plan/docs/service.py`, `src/cmd/plan/docs/drafting.py`
   - 状態管理: `src/state/io.py`, `src/state/path.py` とその利用箇所
   - 開発ルール: `oracle/dev_rule/ROUTING.md` から関連ファイル
   - 仕様断片: `oracle/tgbt_spec/ROUTING.md` から関連ファイル
...
7. 「意図された完全仕様」を問われても、repo から確定できるのは現状実装・現存 `oracle`・既存テストの範囲だけだと明示する。
