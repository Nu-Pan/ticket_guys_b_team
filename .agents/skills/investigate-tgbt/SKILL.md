...
- 開発ルールや作業手順を知りたいときは、`AGENTS.md` と `oracle/docs/dev_rule/*.md` を優先する。
- tgbt の仕様断片を知りたいときは、`oracle/docs/ROUTING.md` と各階層の `ROUTING.md` から関連する `oracle/docs/tgbt_spec/*.md` を辿る。
- `README.md` と `memo/**` は読まない。
- AI 管理ドキュメントや調査メモを作りたくなる作業では `oracle/docs/dev_rule/ROUTING.md` で所在を確認してから `oracle/docs/dev_rule/ai_docs.md` を読み、永続化しない方針を守る。
