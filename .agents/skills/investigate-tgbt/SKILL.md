...
description: tgbt リポジトリ内の事柄について、実装・oracle・残存する既存テストなどの根拠から現状挙動や制約を説明するためのスキル。Use when Codex needs to answer repo-specific questions, trace behavior through code and oracle, inspect existing tests only as historical evidence when present, compare conflicting evidence, summarize the current implementation, or perform scoped investigation without making product-level design decisions.
---
...
- tgbt 自体の開発ではテストを用意しない方針である。tgbt を使った別プロジェクトの開発でテストを用意する方針と混同しない。
...
7. 「意図された完全仕様」を問われても、repo から確定できるのは現状実装・現存 `oracle`・残存する既存テストなど観測可能な成果物の範囲だけだと明示する。
