...
`<tgbt-root>/oracle/**` の既存記述を読み、矛盾、routing の過不足、typo、参照不整合を評価して人間へ返す。
`ROUTING.md` とファイルシステム上の実体との照合は、bundled script で機械的に確認する。
...
6. 明示された記述同士を比較し、評価対象の問題だけを抽出する。不完全性に由来する未記載事項は外す。
7. 判断できないものは推測で埋めず、「未確定」として必要な追加判断を示す。
