---
name: apply-oracle-to-implements
description: Reflect tgbt oracle changes or specified oracle documents into tgbt implementation. Use for implementation-followup work where oracle is authoritative over code, including fixing contradictions, required cascading bugs, and removing implementation that oracle makes obsolete.
---
...
`<tgbt-root>/oracle` の明示内容を正本として扱い、`tgbt` の実装を追従させる。目的は oracle と実装の矛盾解消であり、リファクターや品質改善そのものをゴールにしない。
...
- 人間が oracle ファイル、変更内容、再チェック範囲を指定している場合は、その指示を優先する。
- 指定がない場合は、直近の変更内容に基づく実装追従作業とみなす。
- その場合は、git の未コミット変更と直近数コミットを見て、oracle の意味論的な変更内容を把握する。
- `oracle/docs` を調べるときは各階層の `ROUTING.md` を辿り、必要な `.md` だけ読む。
...
実装計画では、oracle の変更または指定範囲を具体的な実装対象へ対応付ける。対象は関数、クラス、コマンド、入出力など、AI が扱える low-level な単位へ局所化する。

計画の自己レビュー観点:

- oracle の各明示内容が、変更対象ファイルや実装単位へ対応付いているか。
- oracle と実装・既存 state・ユーザー指示の衝突を明示しているか。
...
- 実装結果が oracle の明示内容と矛盾していないか。
- oracle を編集したり、oracle の意味を勝手に読み替えたりしていないか。
- oracle に未記載の仕様判断を最小限に留めているか。
